...
const Expenses = () => {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      window.dispatchEvent(
        new Event('badgeUpdate')
      );
    } catch (err) {
      console.error('Fetch error:', err);

      toast.error(
        'Unable to sync expense records.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const filteredExpenses = expenses
    .filter((exp) => {
      ...
      if (timeFilter === '7 Days') {
        const sevenDaysAgo = new Date().setDate(
          now.getDate() - 7
        );

        matchesTime =
          expDate >= sevenDaysAgo;
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      formData.item_name.length >
      CHAR_LIMIT
    ) {
      toast.error(
        `Item description is too long (Max: ${CHAR_LIMIT} characters).`
      );
      return;
    }
    ...
    try {
      if (editingItem) {
        ...
        await api.put(
          `/expenses/${editingItem._id}`,
          formData
        );

        if (oldName !== newName) {
          toast.success(
            `Item name updated to "${newName}"!`
          );
        } else if (
          oldAmount !== newAmount
        ) {
          toast.success(
            `Amount for "${newName}" updated!`
          );
        } else {
          toast.success(
        ...
        toast.success(
          `Item "${formData.item_name}" added successfully!`
        );
      }
  ...
  const handleArchive = async () => {
    ...
    try {
      await api.put(
        `/expenses/archive/${archiveTarget.id}`
      );

      toast.success(
        `Item "${archiveTarget.name}" moved to archive.`
      );
    ...
    <div className="p-8 space-y-6 min-h-screen bg-[#001226] text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm italic font-bold tracking-wide mt-1 uppercase">
            Track your spending vs labor
            hours.
          </p>
        </div>
            ...
            <tbody className="text-white divide-y divide-white/5 font-medium">
              ...
              {loading ? (
                <tr>
                  <td
                    colSpan="5"
                    className="p-24 text-center text-gray-500 font-black animate-pulse uppercase tracking-[0.3em]"
                  >
                    Syncing...
                  </td>
                </tr>
                        ...
                        <td className="p-8">
                          <div className="flex justify-center">
                            ...
                            <div className="flex items-center gap-6 w-full max-w-[260px]">
                              ...
                              <div
                                className="h-12 w-12 flex-shrink-0 rounded-2xl border flex items-center justify-center shadow-inner"
                                style={{
                                  color:
                                    color,
                              ...
                              <div className="text-left flex flex-col min-w-0">
                                ...
                                <div className="mt-1">
                                  ...
                                  <span
                                    className="inline-block px-2.5 py-0.5 border rounded-full text-[9px] font-black uppercase tracking-widest"
                                    style={{
                                      color:
                                        color,
                        ...
                        <td className="p-8">
                          ...
                          <div className="flex justify-center gap-3">
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 hover:bg-cyan-400 hover:text-[#001226] transition-all font-black text-[10px] uppercase tracking-widest active:scale-95"
                            >
                              Edit
                            ...
                            <button
                              ...
                              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white transition-all font-black text-[10px] uppercase tracking-widest active:scale-95"
                            >
                              Archive
                ...
                <tr>
                  <td
                    colSpan="5"
                    className="p-24 text-center text-gray-600 font-bold uppercase tracking-widest"
                  >
                    No matching records
                    found.
                  </td>
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-[#0a2342] border border-white/5 rounded-xl p-4 text-white font-bold text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/30 shadow-inner"
                    value={
                      formData.item_name
                    }
                    maxLength={
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    min="0.01"
                    placeholder="0.00"
                    className="w-full bg-[#0a2342] border border-white/5 rounded-xl p-4 text-white font-black text-xl focus:outline-none focus:ring-2 focus:ring-cyan-500/30 shadow-inner"
                    value={
                      formData.amount
                    }
                    onChange={(e) =>
                ...
                <div className="pt-4">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-cyan-400 hover:bg-cyan-300 text-[#051930] font-black py-5 rounded-2xl uppercase tracking-widest text-xs shadow-lg transition-all active:scale-95"
                  >
                    {editingItem
                      ? 'Update Record'
                      : 'Submit Record'}
        ...
        {archiveTarget && (
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
            ...
            <motion.div
              ...
              className="w-full max-w-md rounded-[2.5rem] border border-red-500/20 bg-[#05192e] p-8 shadow-[0_0_60px_rgba(239,68,68,0.12)]"
            >
              ...
              <h2 className="text-2xl font-black text-center text-white uppercase tracking-tight">
                Archive Expense?
              </h2>
              ...
              <div className="grid grid-cols-2 gap-4 mt-8">
                ...
                <button
                  ...
                  className="py-4 rounded-2xl border border-white/10 text-white font-black uppercase tracking-widest text-xs hover:bg-white/5 transition-all"
                >
                  Cancel
                </button>
                ...
                <button
                  ...
                  className="py-4 rounded-2xl bg-red-500 text-white font-black uppercase tracking-widest text-xs hover:bg-red-400 transition-all shadow-lg"
                >
                  Confirm
                </button>
              </div>
