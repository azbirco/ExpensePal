...
// DITO ANG PAGBABAGO: Destructuring { protect }
const { protect } = require('../middleware/authMiddleware');

// --- SIDEBAR COUNTS ENDPOINT ---
router.get('/sidebar-counts', protect, async (req, res) => {
    try {
...
router.post('/', protect, async (req, res) => {
    try {
        ...
        if (!item_name || item_name.trim() === "") return res.status(400).json({ message: "Item name is required." });
        if (parseFloat(amount) <= 0) return res.status(400).json({ message: "Amount must be a positive number." });
        if (!category_id) return res.status(400).json({ message: "Please select a category." });
...
router.put('/:id', protect, async (req, res) => {
    try {
        ...
        const user = await User.findById(req.user.id);
        const hourlyRate = (user.monthly_salary || 0) / (user.work_hours_per_month || 160);
        const laborHours = amount / hourlyRate;
        ...
        if (category && (category.category_name.toLowerCase().includes('sav') || category.category_name.toLowerCase().includes('emer'))) {
            await Savings.findOneAndUpdate(
                ...
                { user_id: req.user.id, amount: updatedExpense.amount, description: updatedExpense.item_name },
